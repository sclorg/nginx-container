...
image provides a containerized packaging of the nginx 1.10 daemon. The image can be used 
as a base image for other applications based on nginx 1.10 web server. 
Nginx server image can be extended using Openshift's `Source` build feature.

Usage
-----

For this, we will assume that you are using the `rhscl/nginx-110-rhel7` image, available via `nginx:1.10` imagestream tag in Openshift.
Building a simple [sample-app](https://github.com/sclorg/nginx-container/tree/master/1.10/test/test-app) application
in Openshift can be achieved with the following step:

    ```
    oc new-app nginx:1.10~https://github.com/sclorg/nginx-container.git --context-dir=1.10/test/test-app/
    ```

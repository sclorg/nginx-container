Nginx {{ spec.version }} server and a reverse proxy server container image
============================================================
This container image includes Nginx {{ spec.version }} server and a reverse server for OpenShift and general usage.
...
image provides a containerized packaging of the nginx {{ spec.version }} daemon. The image can be used
as a base image for other applications based on nginx {{ spec.version }} web server.
...
To build a simple [test-app](https://github.com/sclorg/nginx-container/tree/master/examples/{{ spec.version }}/test-app) application in Openshift:

```
oc new-app nginx:{{ spec.version }}~https://github.com/sclorg/nginx-container.git --context-dir={{ spec.version }}/test/test-app/
```
...
podman pull ubi9/nginx-{{ spec.short }}
...
cd nginx-container/examples/{{ spec.version }}/
```
...
```
FROM registry.access.redhat.com/ubi9/nginx-{{ spec.short }}
...
```
$ podman run -d --name nginx -p 8080:8080 -v /wwwdata:/opt/app-root/src:Z ubi9/nginx-{{ spec.short }} nginx -g "daemon off;"
```
